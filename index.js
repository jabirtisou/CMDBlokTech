...
function likeUser(req, res) {
  ...
  User.findById(lUser).then((likingUser) => {
    ...
    likingUser.save().then(newLikingUser => {
      const userId = req.params.userId;
      User.findById(userId).then((likedUser) => {
        likedUser.likedBy.push(lUser);
        likedUser.save().then(newLikedUser => {
...
function dislikeUser(req, res) {
  ...
  User.findById(dUser).then((dislikingUser) => {
    dislikingUser.dislikes.push(dislikes);
    dislikingUser.save().then(newDislikingUser => {
      let dislikedBy = [];
      dislikedBy.push(dUser);
      const userId = req.params.userId;
      User.findById(userId).then(dislikedUser => {
        dislikedUser.dislikedBy.push(dislikedBy);
        dislikedUser.save().then(newDislikedUser => {
          res.redirect("/findmatch");
        }).catch(err => console.log(err.message));
      }).catch(err => console.log(err.message));
    }).catch(err => console.log(err.message));
